...
type Tab = "overview" | "products" | "shops" | "services" | "inquiries" | "ratings" | "reports";
...
export default function BuyerDashboard() {
  ...
  useEffect(() => {
    supabase.auth.getSession().then(async ({ data: { session } }) => {
      ...
      const [{ data: prof }, { data: prods }, { data: shps }, { data: svcs }, { data: inqs }, { data: rats }, { data: reps }] = await Promise.all([
        ...
        supabase.from("ratings").select("*, shops(shop_name), products(name), services(name)").eq("user_id", session.user.id).order("created_at", { ascending: false }),
        supabase.from("reports").select("*, shops(shop_name, shop_number, county)").eq("reporter_id", session.user.id).order("created_at", { ascending: false }),
      ]);
  ...
  const reportStatus: any = {
    pending: { bg: "rgba(245,166,35,0.1)", color: "#b8830a", label: "Under Review" },
    reviewed: { bg: "rgba(59,130,246,0.1)", color: "#2563eb", label: "Reviewed" },
    resolved: { bg: "rgba(37,211,102,0.1)", color: "#168a4e", label: "Resolved" },
    dismissed: { bg: "rgba(13,13,13,0.06)", color: "rgba(13,13,13,0.4)", label: "Dismissed" },
  };

  const renderStars = (n: number) => Array.from({ length: 5 }, (_, i) => (
    <span key={i} style={{ color: i < n ? "#f5a623" : "rgba(13,13,13,0.15)", fontSize: "0.85rem" }}>★</span>
  ));
    ...
    <>
      ...
      <div className="bd-page">
        ...
        <main className="bd-main">
          ...
          {tab === "overview" && (
            <div className="bd-section">
              <div className="bd-section-header">
                ...
                <p>Here is everything you have saved, sent and reviewed on Shoplace</p>
              </div>
              <div className="bd-stats">
                {[
                  ...
                  { ico: "⭐", num: ratings.length, label: "Ratings Given", dest: "ratings" },
                  { ico: "🚩", num: reports.length, label: "Reports Filed", dest: "reports" },
                ].map(s => (
          ...
          {tab === "products" && (
            <div className="bd-section">
              <div className="bd-section-header"><h1>Saved Products</h1><p>{savedProducts.length} product{savedProducts.length !== 1 ? "s" : ""} saved</p></div>
              {savedProducts.length === 0 ? (
                <div className="bd-empty"><div className="bd-empty-ico">📦</div><div className="bd-empty-title">No saved products yet</div><div className="bd-empty-sub">Tap ♡ on any product to save it here.</div><a href="/products" className="bd-cta">Browse Products</a></div>
              ) : (
                    ...
                    return (
                      <div className="fp-card" key={item.product_id}>
                        ...
                        <div className="fp-body">
                          ...
                          <div className="fp-price-row"><span className="fp-price">KSh {p.price?.toLocaleString()}</span>{p.original_price > p.price && <span className="fp-orig">KSh {p.original_price?.toLocaleString()}</span>}</div>
                          <div className="fp-actions">
                            <button className="btn-dark" onClick={() => window.location.href = `/shops/${p.shops?.id}`}>View Shop →</button>
                            <button className="btn-remove" onClick={() => removeProduct(item.product_id)} disabled={removingId === item.product_id}>{removingId === item.product_id ? "..." : "✕ Remove"}</button>
                          </div>
                        </div>
                      </div>
                    );
          ...
          {tab === "shops" && (
            <div className="bd-section">
              <div className="bd-section-header"><h1>Saved Shops</h1><p>{savedShops.length} shop{savedShops.length !== 1 ? "s" : ""} saved</p></div>
              {savedShops.length === 0 ? (
                <div className="bd-empty"><div className="bd-empty-ico">🏪</div><div className="bd-empty-title">No saved shops yet</div><div className="bd-empty-sub">Tap ♡ on any shop to save it here.</div><a href="/shops" className="bd-cta">Browse Shops</a></div>
              ) : (
          ...
          {tab === "services" && (
            <div className="bd-section">
              <div className="bd-section-header"><h1>Saved Services</h1><p>{savedServices.length} service{savedServices.length !== 1 ? "s" : ""} saved</p></div>
              {savedServices.length === 0 ? (
                <div className="bd-empty"><div className="bd-empty-ico">⚙️</div><div className="bd-empty-title">No saved services yet</div><div className="bd-empty-sub">Tap ♡ on any service to save it here.</div><a href="/services" className="bd-cta">Browse Services</a></div>
              ) : (
                  ...
                  {savedServices.map(item => {
                    ...
                    return (
                      <div className="fsvc-card" key={item.service_id}>
                        <div className="fsvc-top"><span className="fsvc-ico">⚙️</span><span className="fsvc-cat">{s.category}</span>{s.shops?.is_verified === true && <span className="vp">✓ Verified</span>}</div>
                        <div className="fsvc-shop">{s.shops?.shop_name}</div>
                        <div className="fsvc-name">{s.name}</div>
                        <div className="fsvc-price">{s.price_type === "free" ? "Free" : s.price_type === "negotiable" ? "Negotiable" : s.price ? `KSh ${s.price.toLocaleString()}${s.price_type === "hourly" ? "/hr" : ""}` : "Price on request"}</div>
          ...
          {tab === "inquiries" && (
            <div className="bd-section">
              <div className="bd-section-header"><h1>Inquiry History</h1><p>{inquiries.length} message{inquiries.length !== 1 ? "s" : ""} sent to sellers</p></div>
              {inquiries.length === 0 ? (
                <div className="bd-empty"><div className="bd-empty-ico">💬</div><div className="bd-empty-title">No inquiries yet</div><div className="bd-empty-sub">When you contact a seller through Shoplace, your messages will appear here.</div><a href="/shops" className="bd-cta">Browse Shops</a></div>
              ) : (
                      ...
                      <div className="inq-card" key={inq.id}>
                        <div className="inq-card-top">
                          <div className="inq-card-left">
                            <div className={"inq-av-lg" + (inq.shops?.is_verified === true ? " inq-av-lg-v" : "")} onClick={() => window.location.href = `/shops/${inq.shops?.id}`}>{inq.shops?.shop_name?.[0]?.toUpperCase()}</div>
                            <div className="inq-card-info">
                              <div className="inq-card-shop" onClick={() => window.location.href = `/shops/${inq.shops?.id}`}>{inq.shops?.shop_name}{inq.shops?.is_verified === true && <span className="vp">✓ Verified</span>}</div>
                              <div className="inq-card-meta">#{String(inq.shops?.shop_number || "").padStart(5, "0")} · {inq.shops?.county}</div>
                        ...
                        <div className="inq-bubble-wrap"><div className="inq-bubble-label">Your message</div><div className="inq-bubble inq-bubble-out">{inq.message}</div></div>
                        {inq.reply ? (
                          <div className="inq-bubble-wrap"><div className="inq-bubble-label">Seller replied · {inq.replied_at ? fmtDate(inq.replied_at) : ""}</div><div className="inq-bubble inq-bubble-in">{inq.reply}</div></div>
                        ) : (
                          <div className="inq-no-reply"><span>⏳</span> Waiting for seller to reply</div>
                        )}
                        <div className="inq-card-actions">
                          {inq.shops?.phone && <a href={`https://wa.me/254${String(inq.shops.phone).replace(/^0/, "")}?text=Hi, following up on my inquiry on Shoplace`} target="_blank" className="btn-wa">💬 Follow up on WhatsApp</a>}
                          {inq.shops?.phone && <a href={`tel:${inq.shops.phone}`} className="btn-call">📞 Call Seller</a>}
          ...
          {tab === "ratings" && (
            <div className="bd-section">
              <div className="bd-section-header"><h1>My Ratings</h1><p>{ratings.length} rating{ratings.length !== 1 ? "s" : ""} submitted</p></div>
              {ratings.length === 0 ? (
                <div className="bd-empty"><div className="bd-empty-ico">⭐</div><div className="bd-empty-title">No ratings yet</div><div className="bd-empty-sub">Rate products, services and shops you have experienced. Your reviews help other buyers.</div>
                  <div className="bd-empty-acts"><a href="/products" className="bd-cta">Browse Products</a><a href="/shops" className="bd-cta-ghost">Browse Shops</a></div>
                </div>
              ) : (
                <div className="ratings-list">
                  {ratings.map(r => {
                    const subject = r.products?.name || r.services?.name || r.shops?.shop_name || "Unknown";
                    const type = r.products ? "Product" : r.services ? "Service" : "Shop";
                    return (
                      <div className="rating-card" key={r.id}>
                        <div className="rating-card-top">
                          <div>
                            <div className="rating-type-badge">{type}</div>
                            <div className="rating-subject">{subject}</div>
                            {r.shops?.shop_name && !r.products && !r.services && null}
                            {(r.products || r.services) && <div className="rating-shop-sub">via {r.shops?.shop_name}</div>}
                          </div>
                          <div className="rating-stars-display">{renderStars(r.stars)}<span className="rating-num">{r.stars}/5</span></div>
                        </div>
                        {r.comment && <div className="rating-comment">"{r.comment}"</div>}
                        <div className="rating-date">{fmtDate(r.created_at)}</div>
                      </div>
                    );
                  })}
                </div>
          ...
          {tab === "reports" && (
            <div className="bd-section">
              <div className="bd-section-header"><h1>My Reports</h1><p>{reports.length} report{reports.length !== 1 ? "s" : ""} filed</p></div>
              {reports.length === 0 ? (
                <div className="bd-empty"><div className="bd-empty-ico">🚩</div><div className="bd-empty-title">No reports filed</div><div className="bd-empty-sub">If you encounter a scam, fake listing or inappropriate content, use the Report button on any shop, product or service.</div>
                  <div className="bd-empty-acts"><a href="/shops" className="bd-cta">Browse Shops</a></div>
                </div>
              ) : (
                <div className="reports-list">
                  {reports.map(r => {
                    const st = reportStatus[r.status || "pending"];
                    return (
                      <div className="report-card" key={r.id}>
                        <div className="report-card-top">
                          <div>
                            <div className="report-shop">{r.shops?.shop_name || "Unknown Shop"}</div>
                            <div className="report-shop-meta">#{String(r.shops?.shop_number || "").padStart(5, "0")} · {r.shops?.county}</div>
                          </div>
                          <div className="report-status-pill" style={{ background: st?.bg, color: st?.color }}>{st?.label}</div>
                        </div>
                        <div className="report-reason-row"><span className="report-reason-label">Reason</span><span className="report-reason">{r.reason}</span></div>
                        {r.details && <div className="report-details">"{r.details}"</div>}
                        <div className="report-date">{fmtDate(r.created_at)}</div>
                      </div>
                    );
                  })}
                </div>
...
.bd-stats{display:grid;grid-template-columns:repeat(3,1fr);gap:1rem;margin-bottom:2.5rem;}
...
.inq-av-lg{width:44px;height:44px;background:var(--rust);border-radius:12px;display:flex;align-items:center;justify-content:center;font-family:'Syne',sans-serif;font-size:1.1rem;font-weight:800;color:white;flex-shrink:0;cursor:pointer;}
...
.inq-status-pill{font-size:0.7rem;font-weight:600;padding:0.25rem 0.75rem;border-radius:100px;flex-shrink:0;}
...
.inq-bubble-out{background:#f0f4f1;border-radius:12px 12px 4px 12px;}
.inq-bubble-in{background:rgba(26,51,38,0.05);border:1px solid rgba(26,51,38,0.1);border-radius:12px 12px 12px 4px;}
...
.rating-card{background:white;border-radius:16px;padding:1.3rem;border:1px solid var(--border);}
.rating-card-top{display:flex;align-items:flex-start;justify-content:space-between;gap:1rem;margin-bottom:0.8rem;}
.rating-type-badge{display:inline-flex;padding:0.14rem 0.55rem;background:rgba(13,13,13,0.05);border-radius:100px;font-size:0.64rem;font-weight:600;color:rgba(13,13,13,0.45);margin-bottom:0.3rem;}
.rating-subject{font-family:'Syne',sans-serif;font-size:0.95rem;font-weight:700;}
.rating-shop-sub{font-size:0.72rem;color:rgba(13,13,13,0.38);margin-top:0.15rem;}
.rating-stars-display{display:flex;align-items:center;gap:0.35rem;flex-shrink:0;}
.rating-num{font-size:0.72rem;color:rgba(13,13,13,0.35);font-weight:500;}
.rating-comment{font-size:0.84rem;color:rgba(13,13,13,0.6);line-height:1.6;font-style:italic;padding:0.75rem 1rem;background:rgba(13,13,13,0.03);border-radius:10px;margin-bottom:0.6rem;border-left:3px solid rgba(245,166,35,0.4);}
.rating-date{font-size:0.68rem;color:rgba(13,13,13,0.28);}
/* REPORTS */
.reports-list{display:flex;flex-direction:column;gap:1rem;}
.report-card{background:white;border-radius:16px;padding:1.3rem;border:1px solid var(--border);}
.report-card-top{display:flex;align-items:flex-start;justify-content:space-between;gap:1rem;margin-bottom:0.8rem;}
.report-shop{font-family:'Syne',sans-serif;font-size:0.95rem;font-weight:700;}
.report-shop-meta{font-size:0.7rem;color:rgba(13,13,13,0.38);margin-top:0.15rem;}
.report-status-pill{font-size:0.7rem;font-weight:600;padding:0.25rem 0.75rem;border-radius:100px;flex-shrink:0;}
.report-reason-row{display:flex;align-items:center;gap:0.6rem;margin-bottom:0.5rem;}
.report-reason-label{font-size:0.7rem;font-weight:600;color:rgba(13,13,13,0.38);text-transform:uppercase;letter-spacing:0.04em;}
.report-reason{font-size:0.84rem;font-weight:500;color:var(--rust);}
.report-details{font-size:0.82rem;color:rgba(13,13,13,0.5);font-style:italic;padding:0.65rem 0.9rem;background:rgba(200,75,49,0.04);border-radius:8px;margin-bottom:0.5rem;border-left:3px solid rgba(200,75,49,0.2);}
.report-date{font-size:0.68rem;color:rgba(13,13,13,0.28);}
/* SHARED BUTTONS */
...
.bd-cta:hover{background:#a83a22;}
...
@media(max-width:960px){
  ...
  .bd-nav-item{flex-direction:column;gap:0.12rem;padding:0.45rem 0.3rem;border-left:none!important;padding-left:0.3rem!important;flex:1;justify-content:center;align-items:center;}
  .bd-nav-item.active{background:rgba(255,255,255,0.09);border-bottom:2px solid #4ade80;}
  .bd-nav-icon{font-size:0.95rem;width:auto;}
  .bd-nav-label{font-size:0.5rem;text-align:center;}
  .bd-nav-count{font-size:0.48rem;padding:0.06rem 0.25rem;}
  .bd-main{padding-bottom:72px;}
  .bd-section{padding:1.4rem 1rem;}
  .bd-stats{grid-template-columns:repeat(3,1fr);gap:0.75rem;}
